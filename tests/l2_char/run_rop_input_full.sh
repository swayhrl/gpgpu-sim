...
out=${TMPDIR:-/tmp}/test_l2char_rop_input_full
set +u
...
"$cxx" -std=c++11 -O2 -I"$CUDA_INSTALL_PATH/include" -I"$root/src" -I"$root" \
  "$root/tests/l2_char/test_rop_input_full.cc" -Wl,--start-group \
...
"$out" "$root/configs/tested-cfgs/SM7_QV100/gpgpusim.config" \
  "$root/tests/l2_char/rop_input_full.config" | tee "$out.log"
grep -q '^L2CHARV1|INVARIANT|slice=0|status=PASS' "$out.log"
grep -Eq 'rop_input_eligible=[1-9][0-9]*' "$out.log"
grep -Eq 'rop_input_blocked=[1-9][0-9]*' "$out.log"
