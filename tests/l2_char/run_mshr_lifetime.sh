...
out=${TMPDIR:-/tmp}/test_l2char_mshr_lifetime
set +u
...
"$cxx" -std=c++11 -O2 -I"$CUDA_INSTALL_PATH/include" -I"$root/src" -I"$root" \
  "$root/tests/l2_char/test_mshr_lifetime.cc" -Wl,--start-group \
...
"$out" "$root/configs/tested-cfgs/SM7_QV100/gpgpusim.config" \
  "$root/tests/l2_char/mshr_lifetime.config" | tee "$out.log"
grep -q '^L2CHARV1|INVARIANT|slice=0|status=PASS' "$out.log"
grep -Eq 'mshr_target_max=2([|]|$)' "$out.log"
grep -Eq 'mshr_lifetime_drain_avg=[1-9]' "$out.log"
