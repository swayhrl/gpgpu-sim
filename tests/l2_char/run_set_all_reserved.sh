...
out=${TMPDIR:-/tmp}/test_l2char_set_all_reserved
...
"$cxx" -std=c++11 -O2 -I"$CUDA_INSTALL_PATH/include" -I"$root/src" -I"$root" \
  "$root/tests/l2_char/test_set_all_reserved.cc" \
  -Wl,--start-group \
  "$build/gpgpu-sim/libgpu_uarch_sim.a" \
  "$build/cuda-sim/libgpgpu_ptx_sim.a" \
...
"$out" "$root/configs/tested-cfgs/SM7_QV100/gpgpusim.config" \
  "$root/tests/l2_char/set_all_reserved.config" | tee "$out.log"

grep -q '^L2CHARV1|INVARIANT|slice=0|status=PASS' "$out.log"
grep -Eq 'max_reserved_ways_any_set=2([|]|$)' "$out.log"
grep -Eq 'cycles_any_set_all_reserved=[1-9][0-9]*' "$out.log"
grep -Eq 'block_set_eligible=[1-9][0-9]*' "$out.log"
grep -Eq 'block_set_blocked=[1-9][0-9]*' "$out.log"
