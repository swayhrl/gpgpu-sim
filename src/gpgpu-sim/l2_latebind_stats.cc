...
void l2_latebind_stats::record_offer(mem_fetch *mf, unsigned long long time,
  ...
  if (request == m_requests.end()) {
    ...
    record.offer_time = time;
    record.accept_time = 0;
    record.probe_status = 0;
    record.access_status = 0;
  ...
  // A request may be offered repeatedly before acceptance, but exactly once
  // afterwards.  Preserve its first offer to expose reservation retry delay.
  assert(!request->second.accepted);
  request->second.accept_time = time;
  request->second.probe_status = probe_status;
  request->second.access_status = access_status;
  request->second.accepted = true;
...
void l2_latebind_stats::record_latency(mem_fetch *mf,
  ...
  assert(it->second.accepted);
  assert(time >= it->second.offer_time);
  ...
  if (it->second.lower_issued && it->second.lower_returned) {
    ...
    m_lower_read_pre_offer_cycles +=
        it->second.lower_issue_time - it->second.offer_time;
