`timescale 1ns/1ps
module tb_gpu_l2_cache_renamed_p31_ooo_retire;
  ...
  reg [TOK_W-1:0] aw_tok0, aw_tok1;
  reg got0, got1;
  reg [7:0] first_retire_id;
  reg got_first_retire;

  gpu_l2_cache_renamed #(.RESP_IN_ORDER(0)) dut(
    .clk(clk),.rst_n(rst_n),
  ...
  always @(posedge clk) begin
    ...
    if (aw_valid && aw_ready) begin
      if (!got0) begin aw_tok0 <= aw_id; got0 <= 1; end
      else if (!got1) begin aw_tok1 <= aw_id; got1 <= 1; end
    end
    if (xbnu_valid && xbnu_ready && !got_first_retire) begin
      first_retire_id <= xbnu_id;
      got_first_retire <= 1;
    end
  end
  ...
  task send_write;
    ...
    begin
      ...
      xbwd_wdata={16{64'hCAFE_0000_0000_0000|id}}; xbwd_wstrb={LINE_W/8{1'b1}};
      xbwd_is_atomic=0;
  ...
  initial begin
    ...
    got0=0; got1=0; got_first_retire=0; first_retire_id=0;

    repeat(5) @(posedge clk); rst_n=1;

    send_write(8'h51,40'h0000_7700);
    send_write(8'h52,40'h0000_7800);

    wait (got0 && got1);

    // Return B in reverse AW token order to test OOO retire.
    @(posedge clk); b_valid=1; b_id=aw_tok1; b_resp=0;
    @(posedge clk); b_valid=0;
    wait (got_first_retire);

    if (first_retire_id != 8'h52) begin
      $display("ERROR expected first retire id=0x52 got=0x%0h", first_retire_id);
      $finish_and_return(1);
    end

    @(posedge clk); b_valid=1; b_id=aw_tok0; b_resp=0;
    @(posedge clk); b_valid=0;

    repeat(10) @(posedge clk);
    $display("RENAMED_P31_OOO_PASS first_retire=0x%0h", first_retire_id);
    $finish;
  end
endmodule
