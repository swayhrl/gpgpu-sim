`timescale 1ns/1ps
module tb_gpu_l2_cache_renamed_p30_flush;
  ...
  always @(posedge clk) begin
    ...
    if (ar_valid && ar_ready) begin
      ...
      r_valid <= 1;
      r_id <= ar_id;
      r_data <= {16{64'h1234_5678_9ABC_DEF0}};
      r_resp <= 0;
    end
  end
  ...
  initial begin
    ...
    xbnd_id=0;xbnd_addr=0;xbnd_bypass=0;xbnd_calg=0;flush_req=0;cache_inv=0;ar_count=0;

    repeat(5) @(posedge clk); rst_n=1;

    send_read(8'h41,40'h0000_6600);
    if (ar_count != 1) begin $display("ERROR first read should miss ar_count=%0d", ar_count); $finish_and_return(1); end

    send_read(8'h42,40'h0000_6600);
    if (ar_count != 1) begin $display("ERROR second read should hit ar_count=%0d", ar_count); $finish_and_return(1); end

    @(posedge clk); flush_req=1;
    @(posedge clk); flush_req=0;
    if (!flush_ack) begin $display("ERROR flush_ack not asserted"); $finish_and_return(1); end

    send_read(8'h43,40'h0000_6600);
    if (ar_count != 2) begin $display("ERROR post-flush read should miss ar_count=%0d", ar_count); $finish_and_return(1); end

    $display("RENAMED_P30_FLUSH_PASS ar_count=%0d", ar_count);
    $finish;
  end
endmodule
